...
public class CommonSuffixLength extends Feature<Attribute>{
	
	public CommonSuffixLength() {
	...
	public FeatureValue apply(Attribute element) {
		...
		for (String attributeValue2 : attributeValues) {
			...
			i = 0;
			commonSuffixLength = 0.0;
			while(i<minLength && attributeValue1.charAt(length1 - 1 - i) == attributeValue2.charAt(length2 - 1 - i)) {
				commonSuffixLength = commonSuffixLength + 1;
				i = i + 1;
			}
			size = size + 1;
			delta = commonSuffixLength - mean;
